...
def extract_features(imgs, cspace='YUV', spatial_size=(32, 32),
    ...
    for i in images_pbar:
        ...
        if cspace != 'RGB':
            ...
            elif cspace == 'GRAY' or cspace == 'GRAYRGB':
                feature_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        else: feature_image = np.copy(image)      
        # Apply bin_spatial() to get spatial color features
        if cspace == 'GRAYRGB':
            rgbfeature_image = np.copy(image)
            spatial_features = bin_spatial(rgbfeature_image, size=spatial_size)
            # Apply color_hist() also with a color space option now
            hist_features = color_hist(rgbfeature_image, nbins=hist_bins, bins_range=hist_range)
        elif cspace != 'GRAY':
        ...
        if visualize:
            if cspace == 'GRAY' or cspace == 'GRAYRGB':
                hog_features, hog_image = get_hog_features(feature_image, orient, 
...
t=time.time()
car_features = extract_features(cars, cspace='GRAYRGB', spatial_size=(32, 32),
                        orient=orient, pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,
                        hist_bins=32, hist_range=(0, 256), hog_channel=-1, datatype='Car')
notcar_features = extract_features(notcars, cspace='GRAYRGB', spatial_size=(32, 32),
                        orient=orient, pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,
                        hist_bins=32, hist_range=(0, 256), hog_channel=-1, datatype='Noncar')
t2 = time.time()
...
carimage, carhog = extract_features([cars[0]], cspace='GRAYRGB', spatial_size=(32, 32),
                        orient=orient, pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,
                        hist_bins=32, hist_range=(0, 256), hog_channel=-1, datatype='Car', visualize=True)
notcarimage, notcarhog = extract_features([notcars[0]], cspace='GRAYRGB', spatial_size=(32, 32),
                        orient=orient, pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,
                        hist_bins=32, hist_range=(0, 256), hog_channel=-1, datatype='Noncar', visualize=True)
