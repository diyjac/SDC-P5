...
We started out with a set of vehicle and non-vehicle images obtained from [GTI vehicle image database](http://www.gti.ssr.upm.es/data/Vehicle_database.html), the [KITTI vision benchmark suite](http://www.cvlibs.net/datasets/kitti/), and examples extracted from the project video itself.  But was unsatisfy with the results obtain with our special use case.  In particular, we all searching for the vehicles top down using a 'birds-eye' view which has many advantages, more on this in *section 2.4, Sliding Windows*.  We will be brief on this, but you can find our abandoned research with the given dataset in [vehicleLab/attampts](./vehicleLab/attampts) and its [README.md](./vehicleLab/attempts/README.md).
